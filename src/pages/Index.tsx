...
// Define types/interfaces instead of using any
interface Route {
...
const Index = () => {
  const navigate = useNavigate();
  const [selectedSegment, setSelectedSegment] = useState<null | unknown>(null);
  const [userRole, setUserRole] = useState<UserRole>('user'); // Default to user role
  const [filters, setFilters] = useState({
    dateRange: 'today',
    distressLevel: 'all',
  ...
  const [end, setEnd] = useState<[number, number] | null>(null);
  const [route, setRoute] = useState<[number, number][]>([]);
  const [allRoutes, setAllRoutes] = useState<Route[]>([]); // Use Route type
  ...
  // Wrap loadUserProfile with useCallback to avoid missing dependency in useEffect
  ...
  // Polyline decoding helper - use const for variables never reassigned
  function decodePolyline(encoded: string): [number, number][] {
  ...
  // ...handlePlanRoutes, handleHighwaySearch, handleImageUpload, etc.
